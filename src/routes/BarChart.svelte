...
<div class="w-2/3 lg:w-1/2 xl:w-1/3">
    	{#each proximity as party}
        	...
        	<div class="overflow-hidden h-2 md:h-3 mb-4 text-xs flex rounded bg-gray-200" style="position: relative;">
            		<div style="left: calc(50% + {party.proximity >= 0 ? '0%' : '-50% - ' + Math.abs(party.proximity * 50) + '%'}); width: {Math.abs(party.proximity * 50)}%;" class="{partyColors[party.party]} opacity-75"></div>
        	</div>
    	{/each}
</div>
