...
<div class="w-2/3 lg:w-1/2 xl:w-1/3">
    	{#each proximity as party}
			...
			<div style="position: absolute; left: 50%; transform: translateX({(party.proximity < 0 ? party.proximity * 50 : 0)}%); width: {Math.abs(party.proximity * 50)}%;" class="{partyColors[party.party]} opacity-75"></div>
        	</div>
    	{/each}
</div>
