...
{#each info.proposals as proposal}
    <div class="shadow overflow-hidden sm:rounded-lg mb-4 w-full">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200 w-full">
            <h2 class="text-lg leading-6 font-medium text-gray-900">{proposal.split(':')[0]}</h2>
            <p class="mt-1 max-w-2xl text-sm text-gray-500">{proposal.split(':')[1]}</p>
        </div>
    </div>
{/each}
	<svelte:fragment slot="footer">
		<a href={info.link}>Link para as propostas</a>
	</svelte:fragment>
</Modal>
